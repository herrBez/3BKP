...
Un problema alternativo che è  stato analizzato è quello di considerare le dimensioni degli zaini come delle variabili, con un upper bound fissato ($S_k^0, S_k^1, S_k^2$), in cui lo scopo è quello di trovare le dimensioni minime che gli zaini devono avere, affinchè contengano tutti gli oggetti.
...
\end{equation}
\subsubsection{Vincoli}
Avendo esteso $\rho$ su tre dimensioni tutti i vincoli che coinvolgono questa variabile devono essere modificati tenendo in considerazione anche l'indice k.
...
(\ref{eq:constraint:volume}) non può più essere considerato, ma poiché è ridondante rispetto ai vincoli (\ref{eq:constraint:coodinateLessThanSkdelta}), che assumono ora la forma:
...
\end{equation}
può essere tralasciato senza intaccare l'ammissibilità del problema.\footnote{Nel paper originale il vincolo (\ref{eq:constraint:volume}) viene considerato esclusivamente per questioni numeriche.}.
...
\end{equation}
che può essere codificato anche nel seguente vincolo:
\begin{equation}
\sigma_k^\delta \leq S_k^\delta \cdot z_k \quad \forall k \in K, \forall \delta \in \Delta
\end{equation}
